...
if( !class_exists( 'vx_crmperks_notice_vxcf_sales' )):

class vx_crmperks_notice_vxcf_sales extends vxcf_sales{
    
public $plugin_url="https://www.crmperks.com";
public $review_link='https://wordpress.org/support/plugin/cf7-salesforce/reviews/?filter=5#new-post';
...
$msg =sprintf(__('Want to save contact form 7 submissions? Manage contact form 7 entries, View and Resend any entry to Salesforce with free %sConatct Form Entries Plugin%s','crm-perks-forms'),'<a href="https://wordpress.org/plugins/contact-form-entries/" target="_blank">','</a>');
?>
<style type="text/css">
.vx_pro_version .fa{
...
<p><i class="fa fa-check"></i> View Salesforce status(sent or not sent) of any entry.</p>
<p><i class="fa fa-check"></i> Easily Resend any entry to Salesforce.</p>
...
public function review_notice() { 
 ...
 if(!empty($install_time) && is_array($install_time) && !empty($install_time['time']) && empty($install_time['review_closed'])){
  ...
  <p><?php echo sprintf(esc_html__( 'You\'ve been using Contact Form 7 salesforce Plugin for some time now; we hope you love it!.%s If you do, please %s leave us a %s rating on WordPress.org%s to help us spread the word and boost our motivation.','contact-form-entries'),'<br/>','<a href="'.esc_url($this->review_link).'" target="_blank" rel="noopener noreferrer">','&#9733;&#9733;&#9733;&#9733;&#9733;','</a>'); ?></p>
...
<p><i class="fa fa-check"></i> Salesforce Custom Objects.</p>
<p><i class="fa fa-check"></i> Upload attachments to "Files" section of Salesforce.</p>
<p><i class="fa fa-check"></i> Assign account to a contact in salesforce.</p>
<p><i class="fa fa-check"></i> Assign contact to any object in salesforce.</p>
<p><i class="fa fa-check"></i> Add a contact and lead to Salesforce Campaign.</p>
...
<p><b><a href="https://wordpress.org/plugins/support-x/" target="_blank">Support X - Wordpress Helpdesk</a></b> Shows user tickets from HelpScout, salesforce, FreshDesk, Desk.com and Teamwork in wordpress. Users can create new tickets and reply to old tickets from wordpress.</p>
...
new vx_crmperks_notice_vxcf_sales();
endif;
